    ...
    socketServer.on('connection', async socket => {
        ...
        socket.on('product', async data => {
            ...
            try {
                ...
                if(!checkProduct) {
                    return socket.emit('message', { status: 'error', message:"El producto no tiene todas la propiedades!" })
                }
                ...
                if (!(typeof title === 'string' && 
                        ...
                        return socket.emit('message', { status: 'error',message:'Tipo de Propiedad no válida' })

                if (data.price < 0 || data.stock < 0) {
                    return socket.emit('message', { status: 'error', message: 'El Producto o stock no pueden tener valores cero o inferior a cero' })
                }
